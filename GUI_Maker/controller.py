...
class Utilities():
	...
	def getTable(self, tableLabel):
		table = self.get(tableLabel, typeList = [handle_WidgetTable])
		return table

	def getCanvas(self, canvasLabel):
...
class handle_Container_Base(handle_Base):
	...
	def onToggleEnable(self, event, *args, **kwargs):
		self.toggleEnable(*args, **kwargs)
		
		#Kill the event so it does not run twice
		# event.Skip()
	...
	def checkShown(self, label = None, **kwargs):
		...
		if (label == None):
			for handle in self[:]:
				answer = handle.checkShown(**kwargs)
		else:
			#Account for multiple objects
			if ((not isinstance(label, list)) and (not isinstance(label, tuple))):
				labelList = [label]
	...
	def checkHidden(self, label = None, **kwargs):
		...
		if (label == None):
			for handle in self[:]:
				answer = handle.checkHidden(**kwargs)
		else:
			#Account for multiple objects
			if ((not isinstance(label, list)) and (not isinstance(label, tuple))):
				labelList = [label]
	...
	def modify(self, label = None, **kwargs):
		...
		if (label == None):
			...
			for label in labelList:
				handle = self.get(label, checkNested = True)
				handle.modify(**kwargs)

	def onSetModified(self, event, *args, **kwargs):
		self.setModified(*args, **kwargs)
	...
	def setModified(self, label = None, **kwargs):
		...
		if (label == None):
			for handle in self[:]:
				answer = handle.setModified(**kwargs)
		else:
			#Account for multiple objects
			if ((not isinstance(label, list)) and (not isinstance(label, tuple))):
				labelList = [label]
			...
			for label in labelList:
				handle = self.get(label, checkNested = True)
				handle.setModified(**kwargs)

	def onCheckModified(self, event, *args, **kwargs):
		self.checkModified(*args, **kwargs)
	...
	def checkModified(self, label = None, **kwargs):
		...
		if (label == None):
			for handle in self[:]:
				answer = handle.checkModified(**kwargs)
		else:
			#Account for multiple objects
			if ((not isinstance(label, list)) and (not isinstance(label, tuple))):
				labelList = [label]
	...
	def readOnly(self, label = None, **kwargs):
		...
		if (label == None):
			...
			for label in labelList:
				handle = self.get(label, checkNested = True)
				handle.readOnly(**kwargs)

	def onSetReadOnly(self, event, *args, **kwargs):
		self.setReadOnly(*args, **kwargs)
	...
	def setReadOnly(self, label = None, **kwargs):
		...
		if (label == None):
			for handle in self[:]:
				answer = handle.setReadOnly(**kwargs)
		else:
			#Account for multiple objects
			if ((not isinstance(label, list)) and (not isinstance(label, tuple))):
				labelList = [label]
...
class handle_Widget_Base(handle_Base):
	...
	def postBuild(self, argument_catalogue):
		...
		#Unpack arguments
		buildSelf, selected, hidden, flags, flex = self.getArguments(argument_catalogue, ["self", "selected", "hidden", "flags", "flex"])
...
class handle_Menu(handle_Container_Base):
	...
	def addMenuItem(self, text = "", icon = None, internal = False, special = None, check = None, default = False, toolTip = "",
		...
		self.betterBind(wx.EVT_MENU, handle.thing, myFunction, myFunctionArgs, myFunctionKwargs)
		# self.thing.Bind(wx.EVT_MENU, myFunction, handle.thing)
...
class handle_MenuPopup(handle_Container_Base):
	...
	def postBuild(self, argument_catalogue):
		...
		self.nested = True #This is not nested. It is called by handle_MenuPopup.onTriggerPopupMenu() or handle_Window.triggerPopupMenu()

	def getValue(self, event = None):
		"""Returns what the contextual value is for the object associated with this handle."""

		if (self.type == "PopupMenu"):
			pass
	...
	def addMenuItem(self, text = "", icon = None, internal = False, special = None, 
		check = None, default = False, toolTip = "", overwrite = True,
	...
	def onTriggerPopupMenu(self, event, preFunction = [None, None, None], postFunction = [None, None, None]):
		...
		popupMenu = self.MyPopupMenu(self, preFunction, postFunction)
		self.myWindow.thing.PopupMenu(popupMenu.thing, position)

		#Destroy the popup menu in memory
		popupMenu.thing.Destroy()
	...
	class MyPopupMenu():
		...
		def populateMenu(self):
			...
			if (len(self.parent.contents) != 0):
				for i, handle in enumerate(self.parent.contents):
					if (handle.text != None):
						self.myMenu.addMenuItem(text = handle.text, icon = handle.icon, internal = handle.internal, special = None, check = handle.check, default = handle.default,
						...
						self.myMenu.addMenuSeparator(label = handle.label, hidden = handle.hidden)
...
class handle_Sizer(handle_Container_Base):
	...
	def addListFull(self, choices = [], default = False, singleSelect = False, editable = False,
		...
		if (drop):
			dropTarget = handle.DragTextDropTarget(handle.thing, dropIndex,
...
class handle_Window(handle_Container_Base):
	...
	def build(self, argument_catalogue):
		...
		#Make the frame
		size, position, autoSize, icon, initFunction, delFunction, endProgram, idleFunction = self.getArguments(argument_catalogue, ["size", "position", "autoSize", "icon", "initFunction", "delFunction", "endProgram", "idleFunction"])
	...
	def setValue(self, label, newValue, *args, **kwargs):
		...
		handle = self.get(label, *args, **kwargs)
		value = handle.setValue(newValue)
		return value
	...
	def setSelection(self, label, newValue, *args, **kwargs):
		...
		handle = self.get(label, *args, **kwargs)
		value = handle.setSelection(newValue)
		return value
